...
impl Topogaphy {
    ...
    fn ascending_neighbors(&self) -> Vec<Topogaphy> {
        vec![
            Topogaphy {row_num: self.row_num+1, col_num: self.col_num, height: self.height+1 },
            Topogaphy {row_num: self.row_num-1, col_num: self.col_num, height: self.height+1 },
            Topogaphy {row_num: self.row_num, col_num: self.col_num+1, height: self.height+1 },
            Topogaphy {row_num: self.row_num, col_num: self.col_num-1, height: self.height+1 }
        ]
    }
}
...
impl LavaProductionFacility {
    fn from_input(input: &str) -> LavaProductionFacility {
        ...
        let map: HashSet<Topogaphy>  = input
            .lines()
            .enumerate()
            .flat_map(|(i, ln)| ln.chars().enumerate().map(move |(j, c)| Topogaphy::build(i, j, c)))
            .inspect(|p| if p.height == 0 {starting_positions.push(*p);})
            .collect();
        LavaProductionFacility { map, starting_positions }
    }

    fn routes<'a>(&self, node: &Topogaphy) -> HashSet<Vec<Topogaphy>> {
        match node.height {
            9 => {
                HashSet::from([vec![*node]])
            }
            _ => {
                node.ascending_neighbors()
                    .into_iter()
...
pub fn part1(input: String) -> Result<String, error::Error> {
    ...
    let result: usize = map
        ...
        .map(|p| {
            ...
            let distinct_9s: HashSet<Topogaphy> = result
                .iter()
